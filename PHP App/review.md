...
```
textタイプのinputタグ内に"123"という値、textareaタグの入力欄に入っている値。上記の場合は「焼肉」が格納されている。
...
返り値は、PDOクラスのインスタンス（DSN、ユーザー名、パスワード）。
PHPとデータベースサーバーの接続をする為に記述している。

### `try catch`とは何か説明してください。
状況次第で発生する可能性があるエラーが発生した時に、正しく動くように対処する処理を例外処理と言い、例外処理を実装する時に使用する。`try`には例外が発生する可能性のあるコード、`catch`には例外が発生した場合に行う処理を入れる。
例外処理が必要な例）名前を指定してファイルを開く処理で、指定した名前のファイルが存在しない時。
...
fetchAllメソッドを使って、todosテーブル内の全てのレコードを返している。
